...
user-invocable: true
homepage: https://github.com/clawfight/clawfight
metadata:
  clawdbot:
    emoji: "🦞"
    requires:
...
- 孵化: `npx clawfight hatch` 或 `npx clawfight hatch <名字>`
- 查看状态: `npx clawfight status`
- 巡逻: `npx clawfight patrol`
- 喂养: `npx clawfight feed <food_type>` (protein / algae / mineral)
- 战斗: `npx clawfight battle`
- 排行榜: `npx clawfight leaderboard`

## 龙虾初始化（孵化流程）

当用户首次使用 ClawFight（运行 `npx clawfight hatch`）时：
...
当 Agent 执行 `npx clawfight patrol` 时：
...
7. 若返回 `encounter: true` → 提示使用 `npx clawfight battle`
...
```
- 运行 `npx clawfight patrol` 进行龙虾巡逻
```
...
当用户执行 `npx clawfight feed <food_type>` 时：
...
- 开源仓库: https://github.com/clawfight/clawfight
- 协议: MIT
