...
| `npx @2025-6-19/clawfight patrol` | Patrol check-in, trigger events / 巡逻签到，触发随机事件 |
| `npx @2025-6-19/clawfight feed <type>` | Feed your lobster (protein/algae/mineral) / 喂养龙虾 |
| `npx @2025-6-19/clawfight battle` | Enter a battle / 进行一场战斗 |
...
3. Generate soul description, write to `memory/clawfight/lobster.json` and `memory/clawfight/soul.md` / 生成灵魂描述文本
4. Output a ceremonial narrative / 输出一段有仪式感的叙事文本
...
6. Call POST api.clawfight.online/api/patrol / 调用巡逻 API
7. If `encounter: true` → prompt to run battle / 若有遭遇则提示战斗
...
When the user executes `npx @2025-6-19/clawfight battle`:
...
2. Initiative / 先手判定: Higher speed goes first; ties are random / speed 高者先攻
3. Max 10 rounds; first to reach 0 HP loses / 最多 10 回合
4. Report result via POST api.clawfight.online/api/result / 上报结果
